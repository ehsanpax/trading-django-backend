...
class MT5Connector(BaseServiceClient):
    ...
    def connect(self, password: str) -> MessageOutputData:
        ...
        if account_info and account_info.login == self.account_id:
            ...
            return MessageOutputData(
                ...
                message=f"Already logged into MT5 account {self.account_id}",
            )
        ...
        if not login_status:
            ...
            return MessageOutputData(
                message_type="error",
                message=f"Failed to log in to MT5: {error_code} - {error_message}",
            )

        print("Successfully Logged into MT5")
        return MessageOutputData(
            message_type="errsucessor",
            message=f"Logged into MT5 account {self.account_id}",
        )
    ...
    def place_trade(self, symbol: str, lot_size: float, direction: str, stop_loss: float, take_profit: float) -> TradeOutputData:
        ...
        if mt5.terminal_info() is None:
            return TradeOutputData(message_type="error", message="⚠️ MT5 session lost before trade execution!")

        # Ensure we are still logged in before placing a trade
        account_info = mt5.account_info()
        if account_info is None or account_info.login != self.account_id:
            return TradeOutputData(message_type="error", message="⚠️ MT5 session lost! Please reconnect before trading.")

        order_type = mt5.ORDER_TYPE_BUY if direction.upper() == "BUY" else mt5.ORDER_TYPE_SELL
        price_info = mt5.symbol_info_tick(symbol)

        if not price_info:
            return TradeOutputData(message_type="error", message=f"⚠️ Symbol {symbol} not found or no tick info available")
        ...
        if result is None:
            ...
            error_code, error_message = mt5.last_error()
            return TradeOutputData(message_type="error", message=f"❌ Trade execution failed: {error_code} - {error_message}")

        if result.retcode != mt5.TRADE_RETCODE_DONE:
            return TradeOutputData(message_type="error", message=f"❌ Trade failed: {result.comment}")

        print("✅ Trade Executed Successfully!")
        return TradeOutputData(
    ...
    def get_account_info(self) -> AccountInfoOutputData:
        ...
        if mt5.terminal_info() is None:
            return AccountInfoOutputData(message_type="error", message="MT5 terminal is not running")

        account_info = mt5.account_info()
        if not account_info:
            return AccountInfoOutputData(message_type="error", message="Failed to retrieve account info from MT5")

        return AccountInfoOutputData(
    ...
    def get_live_price(self, symbol: str) -> CurrentPriceOutputData:
        ...
        if tick:
            return CurrentPriceOutputData(symbol=symbol, bid=tick.bid, ask=tick.ask)
        return CurrentPriceOutputData(message_type="error", message="Failed to retrieve price data")

    def get_symbol_info(self, symbol: str) -> SymbolInfoOutputData:
        """Fetches pip size, tick size, and contract size for a symbol."""
        symbol_info = mt5.symbol_info(symbol)
        if not symbol_info:
            return SymbolInfoOutputData(message_type="error", message=f"Symbol {symbol} not found")
        return SymbolInfoOutputData(
            symbol=symbol,
    ...
    def close_trade(self, ticket: int, volume: float, symbol: str) -> MessageOutputData:
        ...
        if not price_info:
            return MessageOutputData(
                message_type="error", message=f"Price not available for {symbol}"
            )                     
        ...
        if not position:
            return MessageOutputData(
                message_type="error", message=f"No position found for ticket {ticket}"
            )                     
        ...
        if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
            return MessageOutputData(
                message_type="error", message=f"Failed to close trade: {result.comment if result else 'no result'}"
            ) 
            ...
            if deals:
                for deal in deals:
                    if deal.position_id == ticket:
                        ...
                        return TradeDealProfitOutputData(profit=deal.profit)

            print(f"⏳ Retry {attempt+1}/{max_retries} — waiting for closed deal...")
            time.sleep(delay)
        return TradeDealProfitOutputData(
            message_type="error", message=f"No closed deal found for ticket {ticket} after {max_retries} retries"
        )
    ...
    def get_closed_trade_profit(self, order_ticket: int, max_retries=10, delay=2) -> TradeDealProfitOutputData:
        ...
        for attempt in range(max_retries):
            ...
            if deals:
                ...
                return TradeDealProfitOutputData(profit=total_profit)
            print(f"⏳ Retry {attempt+1}/{max_retries} — waiting for closed deals...")
            time.sleep(delay)
        return TradeDealProfitOutputData(
            message_type="error", message=f"No closed deals found for order {order_ticket} after {max_retries} retries"
        )
