...
class MT5Connector:
    """
    Service class for handling MT5 connections and trade operations.
    """
    ...
    def connect(self, password: str):
        ...
        if account_info and account_info.login == self.account_id:
            logger.info(f"Already logged in as {self.account_id}, skipping re-login.")
    ...
    def place_trade(self, symbol: str, lot_size: float, direction: str, stop_loss: float, take_profit: float):
        logger.info("Checking MT5 session before trade...")
        if mt5.terminal_info() is None:
            return {"error": "MT5 session lost before trade execution!"}
        account_info = mt5.account_info()
        if account_info is None or account_info.login != self.account_id:
            return {"error": "MT5 session lost! Please reconnect before trading."}
        order_type = mt5.ORDER_TYPE_BUY if direction.upper() == "BUY" else mt5.ORDER_TYPE_SELL
        price_info = mt5.symbol_info_tick(symbol)
        if not price_info:
            return {"error": f"Symbol {symbol} not found or no tick info available"}
        ...
        logger.info("Sending trade request...")
