...
"""
EMA Crossover Strategy v2 (ema_crossover_v2.py)
-------------------------------------------------
...
class EMACrossover(BaseStrategy):
    ...
    DISPLAY_NAME = "EMA Crossover Strategy v2"
    PARAMETERS = [
        BotParameter(
    ...
    def _place_trade_signal(self, current_price: float, direction: Literal["BUY", "SELL"], 
        ...
        trade_details = {
            ...
            "comment": f"EMACrossoverV2 {self.instrument_symbol} {direction} @ {timestamp}", # Original name for comment
            "strategy_info": {
    ...
    def run_tick(self, df_current_window: pd.DataFrame, account_equity: float) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"[{self.instrument_symbol}] DataFrame columns: {df.columns.tolist()}")

        # Define column names based on the strategy's parameters.
        # This must match the naming convention in BaseStrategy._calculate_indicators
        ema_short_col = f"EMA_ema_length_{self.ema_short_period}_source_close"
        ema_long_col = f"EMA_ema_length_{self.ema_long_period}_source_close"
        atr_col = f"ATR_atr_length_{self.atr_length}"
