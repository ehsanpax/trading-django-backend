...
class OpenPositionsLiveView(APIView):
    ...
    def get(self, request, account_id):
        ...
        if account.platform == "MT5":
            ...
            client = MT5APIClient(settings.MT5_API_BASE_URL, request.auth.key)

        
            login_result = client.post("connect/", {
            ...
            mt5_positions_result = client.get("position/")
            ...
            db_trades_dict = {
                t_data['order_id']: t_data for t_data in serialized_db_trades if t_data.get('order_id') is not None
            }

            for mt5_pos_live in platform_positions_raw:
                ...
                if matched_db_trade:
                    ...
                    merged_data["trade_id"] = matched_db_trade.get("id")
            ...
            for pos in platform_positions_raw:
                ...
                if ticket is not None and ticket not in db_trade_order_ids:
                    ...
                    pos_data = {
                        ...
                        "profit_target": pos.get("takeProfit"),
                        "swap": pos.get("swap"),
        ...
        for account in user_accounts:
            ...
            if account.platform == "MT5":
                ...
                db_trades_user_dict = {
                    ...
                    if t_data.get('order_id') is not None and t_data.get('account') == str(account.id)
                }
                ...
                for mt5_pos_live in platform_positions_raw:
                    ...
                    for i, existing_pos in enumerate(final_all_open_positions):
                        if existing_pos.get("order_id") == ticket and existing_pos.get("account") == str(account.id):
                            merged_data = {**existing_pos, **live_pos_data}
                            merged_data["trade_id"] = existing_pos.get("id")
                            merged_data["order_id"] = ticket
                            merged_data["source"] = "platform_synced_with_db"
                            merged_data["current_pl"] = mt5_pos_live.get("profit")
                            merged_data["stop_loss"] = mt5_pos_live.get("sl")
